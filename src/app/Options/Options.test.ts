...
arr[0] = {
  ...
  result : {_type: 1, _direction: null, _start: -25, _end: 100, _step: 25, _scalestep: 10, _hasTagmark: true, _prefix: '%', _color: null},
};

arr[1] = {
  options : {start: 250, end: 100, step: 300, scalestep: 1000},
  result : {_type: 1, _start: 0, _end: 100, _step: 100, _hasTagmark: true, _scalestep: 100, _color: null, _direction: null, _prefix: null},
};

arr[2] = {
  options : {type: 'double', start: 250, end: 100, values: ['one', 'two', 'three', 4], direction: 'vertical'},
  result : {_type: 2, _direction: 'vertical', _values: ['one', 'two', 'three'], _hasTagmark: true, _color: null, _prefix: null},
};

arr[3] = {
  options : {type: 'double', start: 250, end: 100, values: ['one', 'two', 'three', 4], direction: 'vertical', hasTagmark: false, color: 'red'},
  result : {_type: 2, _direction: 'vertical', _values: ['one', 'two', 'three'], _color: 'red', _hasTagmark: false, _prefix: null},
};
arr[4] = {
  options : {start: 250, end: 100, scalestep: 1000},
  result : {_type: 1, _start: 0, _end: 100, _step: 1, _hasTagmark: true, _scalestep: 100, _color: null, _direction: null, _prefix: null},
};
...
test('Set correct options', ( ) => {
  for (let i=0; i<arr.length; i++) {
    ...
    expect(param.hasTagmark).toBe(arr[i].result._tagmark);
